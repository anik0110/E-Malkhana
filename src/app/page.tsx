...
import { Shield, ChevronRight, BarChart3, Database, FileText, Mail, Phone, MapPin, Menu, X } from 'lucide-react';
...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] font-sans text-slate-900">
      ...
      <nav className="fixed w-full z-40 bg-white/90 backdrop-blur-md border-b border-slate-200">
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="bg-slate-900 p-2 rounded-lg"><Shield className="h-6 w-6 text-white" /></div>
          ...
          <button onClick={() => setAuthModalOpen(true)} className="hidden md:block bg-slate-900 text-white px-5 py-2.5 rounded-lg text-sm font-semibold hover:bg-slate-800 transition shadow-sm">
            Officer Login
          </button>
        ...
        {isMobileMenuOpen && (
          <div className="md:hidden bg-white border-b border-slate-200 px-6 py-4 space-y-4 shadow-lg absolute w-full">
            <a href="#about" className="block text-sm font-medium text-slate-600" onClick={() => setMobileMenuOpen(false)}>About</a>
            <a href="#features" className="block text-sm font-medium text-slate-600" onClick={() => setMobileMenuOpen(false)}>Modules</a>
            <a href="#contact" className="block text-sm font-medium text-slate-600" onClick={() => setMobileMenuOpen(false)}>Contact</a>
            <button onClick={() => { setAuthModalOpen(true); setMobileMenuOpen(false); }} className="w-full bg-slate-900 text-white px-5 py-3 rounded-lg text-sm font-semibold">
              Officer Login
            </button>
          </div>
        )}
      ...
      <section className="pt-32 pb-16 md:pb-24 px-6 bg-gradient-to-b from-slate-50 to-white">
        ...
        <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-2 gap-12 md:gap-16 items-center">
          <div className="relative group order-2 md:order-1">
             ...
             <img 
               ...
               alt="Law and Order" 
               className="relative rounded-lg shadow-xl border border-slate-100 w-full" 
             />
          </div>
          <div className="order-1 md:order-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mt-8">
               <div className="border-l-4 border-indigo-600 pl-4">
                  ...
                  <p className="text-sm text-slate-500 mt-1">Immutable digital logs.</p>
               </div>
               <div className="border-l-4 border-emerald-500 pl-4">
                  <h4 className="font-bold text-slate-900">Fast Retrieval</h4>
                  <p className="text-sm text-slate-500 mt-1">QR Code scanning.</p>
               </div>
            </div>
          </div>
        </div>
      ...
      <section id="features" className="py-16 md:py-24 px-6 bg-slate-50">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-12 md:mb-16">
            <h2 className="text-3xl font-bold text-slate-900">Core Modules</h2>
            <p className="text-slate-500 mt-4">Designed for operational efficiency.</p>
          </div>
      ...
      <footer id="contact" className="bg-slate-900 text-slate-300 py-12 md:py-16 px-6 border-t border-slate-800">
...
function FeatureCard({ icon: Icon, title, desc }: any) {
  return (
    <div className="bg-white p-6 md:p-8 rounded-xl border border-slate-200 shadow-sm hover:shadow-md transition-all duration-300 group">
