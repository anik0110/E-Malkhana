...
export default function ManageCases() {
  ...
  const handleSearch = (e: React.FormEvent) => { e.preventDefault(); fetchCases(query); };
  ...
  return (
    <div className="p-4 md:p-8 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 border-b border-slate-200 pb-6">
        <div>
          <h1 className="text-xl md:text-2xl font-bold text-slate-800">Manage Entries</h1>
      ...
      {/* Search Bar */}
      <form onSubmit={handleSearch} className="flex gap-3 bg-white p-2 rounded-xl border border-slate-200 shadow-sm max-w-2xl w-full">
        <div className="relative flex-1">
          <Search className="absolute left-3 top-3 h-5 w-5 text-slate-400" />
          <input 
            type="text" placeholder="Search by Crime Number or IO Name..." 
            className="w-full pl-10 pr-4 py-2.5 bg-transparent outline-none text-slate-700 placeholder:text-slate-400"
            value={query} onChange={(e) => setQuery(e.target.value)}
          />
        </div>
        <button type="submit" className="bg-slate-900 text-white px-4 md:px-6 py-2 rounded-lg font-medium hover:bg-slate-800 transition">
          Search
        </button>
      </form>
            ...
            <table className="w-full text-left border-collapse min-w-[800px]">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-200">
                  <th className="p-4 md:p-5 font-semibold text-xs text-slate-500 uppercase tracking-wider">Case ID</th>
                  <th className="p-4 md:p-5 font-semibold text-xs text-slate-500 uppercase tracking-wider">Station</th>
                  <th className="p-4 md:p-5 font-semibold text-xs text-slate-500 uppercase tracking-wider">IO Name</th>
                  <th className="p-4 md:p-5 font-semibold text-xs text-slate-500 uppercase tracking-wider">Status</th>
                  <th className="p-4 md:p-5 font-semibold text-xs text-slate-500 uppercase tracking-wider">Date</th>
                  <th className="p-4 md:p-5 font-semibold text-xs text-slate-500 uppercase tracking-wider text-right">Actions</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                {cases.length === 0 ? (
                  <tr><td colSpan={6} className="p-8 text-center text-slate-500 italic">No cases found matching your search.</td></tr>
                ) : (
                  cases.map((c: any) => (
                    <tr key={c._id} className="hover:bg-slate-50/80 transition-colors group">
                      <td className="p-5"><span className="font-mono font-bold text-indigo-600 bg-indigo-50 px-2 py-1 rounded text-xs">#{c.crimeNumber}/{c.crimeYear}</span></td>
                      <td className="p-5 text-sm text-slate-700">{c.stationName}</td>
                      <td className="p-5 text-sm text-slate-700 font-medium">{c.ioName}</td>
                      <td className="p-5">
                        <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wide border ${c.status === 'Active' ? 'bg-amber-50 text-amber-600 border-amber-100' : 'bg-emerald-50 text-emerald-600 border-emerald-100'}`}>{c.status}</span>
                      </td>
                      <td className="p-5 text-sm text-slate-500">{new Date(c.createdAt).toLocaleDateString()}</td>
                      <td className="p-5 flex justify-end gap-2">
                        <Link href={`/cases/${c._id}`} className="p-2 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition"><Eye className="h-4 w-4" /></Link>
                        <button onClick={() => handlePrintReport(c._id)} disabled={printingId === c._id} className="p-2 text-slate-400 hover:text-slate-900 hover:bg-slate-100 rounded-lg transition disabled:opacity-50">
