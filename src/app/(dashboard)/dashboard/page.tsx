...
import { Package, CheckCircle, Clock, Search, ArrowUpRight } from 'lucide-react';
    ...
    <div className="p-8 max-w-7xl mx-auto space-y-8">
      ...
      <div className="flex justify-between items-end">
        <div>
          ...
          <p className="text-slate-500 mt-1">Welcome back, {session?.user?.name || 'Officer'}</p>
        </div>
          ...
          <div className="relative z-10">
            ...
            <div className="space-y-4">
               <div className="flex justify-between text-sm items-center border-b border-white/10 pb-2">
                 ...
                 <span className="text-emerald-400 font-medium flex items-center gap-2"><span className="w-2 h-2 rounded-full bg-emerald-400"></span> Connected</span>
               </div>
               <div className="flex justify-between text-sm items-center border-b border-white/10 pb-2">
...
function ActionCard({ href, title, desc }: any) {
  return (
    <a href={href} className="group p-5 rounded-xl border border-slate-100 bg-slate-50 hover:bg-blue-50 hover:border-blue-100 transition-all">
      <div className="flex justify-between items-start mb-2">
        <h4 className="font-bold text-slate-800 group-hover:text-blue-700">{title}</h4>
